...
def get_ext(inPath, extension):
    ...
    if os.path.isfile(inPath):
        ...
        if extension is None:
            ...
            print('Warning: No --type was specified. By default this will convert EVERY file in \n',
                  os.path.abspath(inPath), '\n and its subdirs, that have the following extensions: ', ext)
            resume = yes_or_no('Do you want to continue?')
        else:
            ext = extension.split()
            suspicious = False
            for i in range(len(ext)):
                e_old = ext[i]
                ext[i] = ext[i].replace(",",' ').replace("[",'').replace("]",'').replace("(",'').replace(")",'')
                ext[i] = ext[i].replace('"','').replace("'",'').replace(' ','')
                if ext[i] != e_old:
                    suspicious = True
            if suspicious:
                print('Warning: Questionable formatting for --type detected. Do you want to continue to convert '
                    'all files in \n', os.path.abspath(inPath), '\n that have the following extensions: ', ext)
                resume = yes_or_no('Do you want to continue?')
    if not resume:
        sys.exit('Aborting... Correct --type formatting should look like this:\n'
            '--type=".dae .stl .obj .urdf"\n'
            'or like this:\n'
            '--type=.urdf')            
    return ext
...
if __name__ == "__main__":
    ...
    optParser.add_option('--outPath', dest='outPath', default=None, help='Where converted obj file should be placed. '
                         ' Only use for converting mesh files in a directory. It will place all converted files in the outPath, '
                         ' ignoring any further recursive folder structure. ')
    optParser.add_option('--type', dest='ext', default=None, help='The file extensions to convert, if --input is a directory. '
                         'Can be list --type=".dae .stl .obj .urdf". If defining multiple file extensions, make sure you '
                         'surround them by quotation marks and seperate them by spaces, as shown in the example.')
    ...
    optParser.add_option('--backup', dest='backup', action='store_true', default=False, help='If set, will create a backup '
                         ' of the "--input" directory next to it.')
